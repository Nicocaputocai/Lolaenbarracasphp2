...
function __construct($inId=null, $inTitle=null, $inPost=null, $inPostFull=null, $inAuthorId=null, $inDatePosted=null,$inCategory=null,$published=null,$inPostPict=null, $inPostDescrip=null)
{
global $mysqli;
    if (!empty($inId))
    {
    ...
    if (!empty($inPostDescrip))
    {
        $this->postDescrip = $inPostDescrip;
    }
    if (!empty($inPost))
    {
        $this->post = $inPost;
...
//
    ...
    if (!empty($inId))
    {
        $query = $mysqli->query("SELECT tags.* FROM blog_post_tags LEFT JOIN (tags) ON (blog_post_tags.tag_id = tags.id) WHERE tags.name IS NOT NULL AND blog_post_tags.blog_post_id = " . $inId);
